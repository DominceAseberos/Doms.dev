...
// ══════════════════════════════════════════════════════════════════════════
// LANDING TAB — landingData.json
// ══════════════════════════════════════════════════════════════════════════
const LandingTab = () => {
    const [d, setD] = useState(() => normalizeLandingData(landingDataDefault));
    const [status, setStatus] = useState(null);
    const timer = useRef(null);

    useEffect(() => {
        fetchLandingData().then(r => setD(normalizeLandingData(r))).catch(() => {});
    }, []);

    const save = async (next) => {
        setStatus('saving');
        try { await saveLandingData(next); setStatus('saved'); setTimeout(() => setStatus(null), 2000); }
        catch { setStatus('error'); }
    };
    ...
    const setHero    = (p) => update(prev => ({ ...prev, hero:    { ...prev.hero,    ...p } }));
    const setConnect = (p) => update(prev => ({ ...prev, story:   { ...prev.story,   connect: { ...prev.story.connect, ...p } } }));
    const setOrigin  = (p) => update(prev => ({ ...prev, story:   { ...prev.story,   origin:  { ...prev.story.origin,  ...p } } }));
    const setHumanS  = (p) => update(prev => ({ ...prev, story:   { ...prev.story,   human:   { ...prev.story.human,   ...p } } }));
    const setArsenal = (p) => update(prev => ({ ...prev, story:   { ...prev.story,   arsenal: { ...prev.story.arsenal, ...p } } }));
    const setWork    = (p) => update(prev => ({ ...prev, story:   { ...prev.story,   work:    { ...prev.story.work,    ...p } } }));
    const setStoryHero=(p) => update(prev => ({ ...prev, story:   { ...prev.story,   hero:    { ...prev.story.hero,    ...p } } }));

    const setMetric = (i, p) => update(prev => { const a=[...prev.metrics]; a[i]={...a[i],...p}; return {...prev,metrics:a}; });
    const setFeatProj = (i, p) => update(prev => { const a=[...prev.story.work.projects]; a[i]={...a[i],...p}; return {...prev,story:{...prev.story,work:{...prev.story.work,projects:a}}}; });
    const setStrength = (i, p) => update(prev => { const a=[...prev.story.arsenal.strengths]; a[i]={...a[i],...p}; return {...prev,story:{...prev.story,arsenal:{...prev.story.arsenal,strengths:a}}}; });
    const setOriginPara = (i, v) => update(prev => { const a=[...prev.story.origin.paragraphs]; a[i]=v; return {...prev,story:{...prev.story,origin:{...prev.story.origin,paragraphs:a}}}; });
    const setHumanFact  = (i, v) => update(prev => { const a=[...prev.story.human.facts]; a[i]=v; return {...prev,story:{...prev.story,human:{...prev.story.human,facts:a}}}; });
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-5">
                <h2 className="text-sm font-bold text-white">Landing / Story Page</h2>
                <div className="flex items-center gap-3">
                    <SaveBadge status={status} />
                    <button onClick={() => save(d)} className="px-4 py-2 text-xs bg-[#c8ff3e] text-black font-bold rounded-lg hover:bg-white transition-colors uppercase tracking-widest">Save Now</button>
                </div>
            ...
            <Card title="Story Hero" icon={<FiUser />} accent="#98c1d9">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
                    <Field label="Full Name"><input value={story.hero?.fullName||''} onChange={e=>setStoryHero({fullName:e.target.value})} className={inputCls} /></Field>
                    <Field label="Role"><input value={story.hero?.role||''} onChange={e=>setStoryHero({role:e.target.value})} className={inputCls} /></Field>
                    <Field label="Tagline" className="md:col-span-2"><textarea value={story.hero?.tagline||''} onChange={e=>setStoryHero({tagline:e.target.value})} rows={2} className={textareaCls} /></Field>
                </div>
            </Card>

            <Card title="Hero Section (Old)" icon={<FiLayout />} accent="#98c1d9">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
                    <Field label="Kicker"><input value={d.hero?.kicker||''} onChange={e=>setHero({kicker:e.target.value})} className={inputCls} /></Field>
                    <Field label="Display Kicker"><input value={d.hero?.displayKicker||''} onChange={e=>setHero({displayKicker:e.target.value})} className={inputCls} /></Field>
                    <Field label="Bio" className="md:col-span-2"><textarea value={d.hero?.bio||''} onChange={e=>setHero({bio:e.target.value})} rows={2} className={textareaCls} /></Field>
                </div>
                <div className="border-t border-white/5 pt-4 mt-2">
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Marquee Words</p>
                    <TagInput items={d.hero?.marquee||[]} onChange={v=>setHero({marquee:v})} placeholder="Add word..." />
                </div>
                <div className="border-t border-white/5 pt-4 mt-4">
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Tags Grid</p>
                    <TagInput items={d.tags||[]} onChange={v=>update({tags:v})} placeholder="Add tag..." />
                </div>
                <div className="border-t border-white/5 pt-4 mt-4">
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Metrics</p>
                    {(d.metrics||[]).map((m,i)=>(
                        <div key={i} className="grid grid-cols-3 gap-2 mb-2">
                            <input value={m.value} onChange={e=>setMetric(i,{value:e.target.value})} className={inputCls} placeholder="Value" />
                            <input value={m.unit}  onChange={e=>setMetric(i,{unit:e.target.value})}  className={inputCls} placeholder="Unit" />
                            <input value={m.label} onChange={e=>setMetric(i,{label:e.target.value})} className={inputCls} placeholder="Label" />
                        </div>
                    ))}
                </div>
            </Card>

            <Card title="Chapter — Origin" icon={<FiFileText />} accent="#98c1d9">
                <Field label="Headline"><input value={story.origin?.headline||''} onChange={e=>setOrigin({headline:e.target.value})} className={inputCls} /></Field>
                {(story.origin?.paragraphs||[]).map((p,i)=>(
                    <Field key={i} label={`Paragraph ${i+1}`}><textarea value={p} onChange={e=>setOriginPara(i,e.target.value)} rows={3} className={textareaCls} /></Field>
                ))}
            </Card>

            <Card title="Chapter — Arsenal" icon={<FiCode />} accent="#98c1d9">
                <Field label="Headline"><input value={story.arsenal?.headline||''} onChange={e=>setArsenal({headline:e.target.value})} className={inputCls} /></Field>
                <Field label="Marquee Tech Items">
                    <TagInput items={story.arsenal?.marqueeItems||[]} onChange={v=>setArsenal({marqueeItems:v})} placeholder="Add tech..." />
                </Field>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 mt-2">Strength Cards</p>
                {(story.arsenal?.strengths||[]).map((s,i)=>(
                    <div key={i} className="grid grid-cols-12 gap-2 mb-2 p-3 bg-black/30 rounded-xl border border-white/5">
                        <div className="col-span-2"><input value={s.symbol} onChange={e=>setStrength(i,{symbol:e.target.value})} className={inputCls} placeholder="🧩" /></div>
                        <div className="col-span-4"><input value={s.title} onChange={e=>setStrength(i,{title:e.target.value})} className={inputCls} placeholder="Title" /></div>
                        <div className="col-span-6"><input value={s.description} onChange={e=>setStrength(i,{description:e.target.value})} className={inputCls} placeholder="Description" /></div>
                    </div>
                ))}
            </Card>

            <Card title="Chapter — Work" icon={<FiCheckSquare />} accent="#98c1d9">
                <Field label="Headline"><input value={story.work?.headline||''} onChange={e=>setWork({headline:e.target.value})} className={inputCls} /></Field>
                {(story.work?.projects||[]).map((p,i)=>(
                    <div key={i} className="mb-3 p-4 bg-black/30 rounded-xl border border-white/5">
                        <p className="text-[10px] font-bold text-[#98c1d9] uppercase mb-3">Featured Project {i+1}</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            <Field label="Title"><input value={p.title} onChange={e=>setFeatProj(i,{title:e.target.value})} className={inputCls} /></Field>
                            <Field label="Link"><input value={p.href} onChange={e=>setFeatProj(i,{href:e.target.value})} className={inputCls} /></Field>
                            <Field label="Description" className="md:col-span-2"><textarea value={p.description} onChange={e=>setFeatProj(i,{description:e.target.value})} rows={2} className={textareaCls} /></Field>
                            <Field label="Tags (comma-separated)" className="md:col-span-2">
                                <input value={(p.tags||[]).join(', ')} onChange={e=>setFeatProj(i,{tags:e.target.value.split(',').map(t=>t.trim()).filter(Boolean)})} className={inputCls} />
                            </Field>
                        </div>
                    </div>
                ))}
            ...
            <Card title="Chapter — Human" icon={<FiUser />} accent="#98c1d9">
                <Field label="Quote"><textarea value={story.human?.quote||''} onChange={e=>setHumanS({quote:e.target.value})} rows={3} className={textareaCls} /></Field>
                {(story.human?.facts||[]).map((f,i)=>(
                    <Field key={i} label={`Fact ${i+1}`}><input value={f} onChange={e=>setHumanFact(i,e.target.value)} className={inputCls} /></Field>
                ))}
            </Card>

            <Card title="Chapter — Connect" icon={<FiMail />} accent="#98c1d9">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
                    <Field label="Headline"><input value={story.connect?.headline||''} onChange={e=>setConnect({headline:e.target.value})} className={inputCls} /></Field>
                    <Field label="Email"><input value={story.connect?.email||''} onChange={e=>setConnect({email:e.target.value})} className={inputCls} /></Field>
                    <Field label="Subtext" className="md:col-span-2"><textarea value={story.connect?.subtext||''} onChange={e=>setConnect({subtext:e.target.value})} rows={2} className={textareaCls} /></Field>
                    <Field label="GitHub URL"><input value={story.connect?.githubUrl||''} onChange={e=>setConnect({githubUrl:e.target.value})} className={inputCls} /></Field>
                    <Field label="LinkedIn URL"><input value={story.connect?.linkedinUrl||''} onChange={e=>setConnect({linkedinUrl:e.target.value})} className={inputCls} /></Field>
                    <Field label="Footer Copyright" className="md:col-span-2"><input value={story.connect?.footerCopyright||''} onChange={e=>setConnect({footerCopyright:e.target.value})} className={inputCls} /></Field>
                </div>
            </Card>
        </div>
    );
