...
import { fetchSocials } from '../../../shared/landingService';
...
const CityscapeContact = () => {
  ...
  // Socials loaded from landingData.json — edit that file to update links
  const [socialSpots, setSocialSpots] = useState(staticLandingData.socials || []);

  useEffect(() => {
    fetchSocials().then(setSocialSpots);
  }, []);
...
const CityscapeSVG = ({ svgRef, lit }) => (
  ...
  >
    ...
    {/* Social Popup Bubbles — data from src/data/landingData.json */}
    <g className="cc-socials-layer" style={{ zIndex: 50 }}>
      {socialSpots.map(spot => (
...
// 📝 To add/edit/remove social links, update src/data/landingData.json
//    Each entry needs: id, label, href, cx, cy, iconPath
  ...
  return (
    ...
    >
      ...
      <g transform={`translate(${spot.cx - 18}, ${spot.cy - 18}) scale(1.5)`}>
        <path d={spot.iconPath} className="cc-social-icon" />
      </g>
    </a>
  );
};
