...
import {
    ...
    SiDocker, SiFramer, SiSvg, SiJavascript, SiDart, SiPhp, SiFlask, SiSqlite,
    SiPytorch, SiKeras, SiOpencv, SiRailway, SiRender, SiPuppeteer,
    SiGooglegemini, SiOllama, SiGit, SiGithub, SiPostman, SiGooglesearchconsole, SiGoogleanalytics
} from 'react-icons/si';
import { Database, Layout, Webhook, Box, Code, Layers, Droplet, Activity, Bug, TestTubes } from 'lucide-react';
...
export const getTechIcon = (name) => {
    ...
    if (n.includes('sql') && !n.includes('sqlite') && !n.includes('postgres')) return <Database />;
    if (n.includes('flask')) return <SiFlask />;
    if (n.includes('websocket') || n.includes('sse')) return <Activity />;
    if (n.includes('sqlite')) return <SiSqlite />;
    if (n.includes('pytorch')) return <SiPytorch />;
    if (n.includes('keras')) return <SiKeras />;
    if (n.includes('opencv')) return <SiOpencv />;
    if (n.includes('mediapipe')) return <Box />;
    if (n.includes('railway')) return <SiRailway />;
    if (n.includes('render')) return <SiRender />;
    if (n.includes('playwright')) return <TestTubes />;
    if (n.includes('puppeteer')) return <SiPuppeteer />;
    if (n.includes('scrapingant')) return <ScrapingAntIcon />;
    if (n.includes('gemini')) return <SiGooglegemini />;
    if (n.includes('ollama')) return <SiOllama />;
    if (n.includes('github') && !n.includes('action')) return <SiGithub />;
    if (n === 'git' || n.includes(' git')) return <SiGit />;
    if (n.includes('postman')) return <SiPostman />;
    if (n.includes('search console')) return <SiGooglesearchconsole />;
    if (n.includes('google analytics')) return <SiGoogleanalytics />;
