...
import adminStrings from '@shared/config/adminStrings.json';
...
const FeedManager = () => {
    ...
    const fetchProfile = async () => {
        try {
            ...
            if (profile) {
                setLiveStatus(profile.live_feed_status || '');
            }
        } catch (error) {
    ...
    const updateLiveStatus = async () => {
        ...
        try {
            ...
            await profileService.updateLiveStatus(profile.id, liveStatus);
    ...
    const handlePost = async () => {
        ...
        try {
            ...
            if (newPostImage) {
                const fileName = `feed_${Date.now()}`;
                imageUrl = await mediaService.uploadImage(newPostImage, 'project-images', fileName);
            }

            const { data: { user } } = await supabase.auth.getUser();
            await postService.createPost(newPostContent, imageUrl, user?.id);
